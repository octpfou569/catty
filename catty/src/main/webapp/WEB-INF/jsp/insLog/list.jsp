						...
						<div class="ins-log">
							...
							<div class="col-md-5">
								<div class="map-inslog-list">
									<div id="resource"></div>
									<input type="button" class="button-move" value="등록" onclick="moveInspectionRegist()" style="float: left; margin-top: 30px;"/>	
								</div>
							</div>
							<div class="col-md-7">
								...
								<div class="row" style="margin-top: 35px;">
									...
									<input type="text" class="text-search" id="term" name="term" placeholder="검색어를 입력하세요"/>
								</div>
							</div>
						</div>
					</div>
		...
		$.ajax({
			...
			success : function(rows){
				...
				for (var i = 0 ; i < rows.inspectionLogList.length; i++) {
					...
					html += "	</td>";
					html += "	<td>";
					html += rows.memberList[i].id + "(" + rows.memberList[i].name + ")";
					html += "	</td>";
					html += "</tr>";
				}
				html +="</table>";
				 ...
				 var script = "";
					...
					if (rows.inspectionLogList.length != null) {
						script += "<div id='map' style='margin-left:18px;width:500;height:470px;'></div>";
