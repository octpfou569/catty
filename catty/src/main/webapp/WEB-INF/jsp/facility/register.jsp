								...
								<div class="col-md-12">
									<form action="/facility" method="post" id="registerForm">
											...
											<div class="info">
												<div class="col-md-5">
													<div class="detail">
														...
														<p>
															<label>고유번호 : </label><input type="text" id="input-uid" name="uid" readonly="readonly" required style="width: 175px"> 
																<input type="button" id="tag" class="button-renew-uid" value="갱신" onclick="renewUid()"> 
														</p>
												...
												<div class="col-md-7">
													<div id="resource" style="margin-bottom: 60px"></div><br>
												</div>
											</div>
										</div>
											...
											<div class="col-md-12">
												<div class="optional">
													...
													<select multiple class="select-member" name="fromBox" id="fromBox">
														<optgroup style="background-color: #dedede; font-size:17px" label="회원 목록">
													...
													<select multiple class="select-manager" name="toBox" id="toBox">
														<optgroup style="background-color: #dedede; font-size:17px" label="담당자">
														</optgroup>
													</select><br><br>
										...
										<div class="row">
											<div class="col-md-12">
												<div class="buttons">
													<input class="button-submit" type="button" value="등록" onclick="addManagementFacilityAndSubmit()"/>
												</div>
											</div>
										</div>
									</form>
...
function renewUid() {
	$.ajax({
        url: "${pageContext.request.contextPath}/facility/-1",
        type: "get",
        headers: { "Content-Type" : "application/json;charset=UTF-8" },
        success: function(map) {
			if (map.facility.uid.trim() != "" && map.facility.uid != null) {
				for (var i = 0; i < map.facilityList; i++) {
					if (map.facilityList[i].uid == map.facilityList.uid) {
						isUsable = false;
						break;
					}
				}
				...
				if (isUsable == true) {
					$('#input-uid').val(map.facility.uid);
					$('#tag').val("성공");
				} else {
					$('#tag').val("실패");
				}
	...
	function drawAccessMap() {
		...
		$.ajax({
	        ...
	        success: function(rows) {
				script += "<div id='map' style='width:100%;height:400px;margin:auto;box-shadow: 0px 0px 10px 2px'></div>";					
