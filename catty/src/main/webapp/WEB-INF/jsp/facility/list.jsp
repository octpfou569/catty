					...
					<div class="facility">
						<div class="list">
							...
							<div class="row">
								<div class="col-md-12">
									<div class="buttons">
											<input class="button-search" type="button" id="btn" value="검색" style="float:right;margin-right:50px;"/>
											<input class="text-search" placeholder="검색어를 입력하세요" type="text" id="term" name="term" style="float:right"/>
											<input class="button-move" type="button" value="등록" onclick="moveRegistFacility()" style="float:left;margin-left: 50px;"/>
									</div>
								</div>
							</div>
						</div>
				 ...
				 var script = "";
					...
					if (rows.facilityList.length != null) {
			           ...
			           for (var i = 0 ; i < rows.facilityList.length; i++) {
			           		...
			           		contentStrings[i] = [
					       	    '<div class="iw_inner">',
					       	  	'<h4>'+rows.facilityList[i].name+'</h4>',
					       	    '</div>'
					       	].join('');
