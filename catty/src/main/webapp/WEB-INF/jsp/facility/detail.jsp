											...
											<div class="info">
												<div class="col-md-4">
													<div class="detail">
															...
															<p><label>고유번호 : </label><input type="text" id="uid" name="uid" value="${facility.uid}" readonly="readonly" style="width: 185px;"></p>
															<p><label>종류 :</label>
																<c:choose>
																...
																<tr>
																	...
																	<c:forEach var="feedBarrel" items="${feedBarrelList}">
																		<tr>
																			...
																			<c:choose>
																				<c:when test="${feedBarrel.supplementStatus eq 'X'}">
																					<td style="text-align: center;color: #00B915;font-size:18px">●</td>
																				</c:when>
																				<c:when test="${feedBarrel.supplementStatus eq 'O'}">
																					<td style="text-align: center;color: red;font-size:18px">●</td>
																				</c:when>
																			</c:choose>	
																...
																<tr>
																	...
																	<c:forEach var="waterBarrel" items="${waterBarrelList}">
																		<tr>
																			...
																			<td>${waterBarrel.standard}g</td>
																			<c:choose>
																				<c:when test="${waterBarrel.supplementStatus eq 'X'}">
																					<td style="color: #00B915">${waterBarrel.capacity}g</td>
																				</c:when>
																				<c:when test="${waterBarrel.supplementStatus eq 'O'}">
																					<td style="color: red">${waterBarrel.capacity}g</td>
																				</c:when>
																			</c:choose>	
																			<c:choose>
																				<c:when test="${waterBarrel.supplementStatus eq 'X'}">
											...
											<div class="col-md-12">
												<div class="detail-buttons">
													<form action="${pageContext.request.contextPath}/facility/${facility.no}" method="post">
														...
														<input type="submit" value="삭제" style="margin-right: 30px;">
													</form>
		...
		$.ajax({
	        ...
	        success: function() {
				...
				if ("${facility.uid}" != null) {
					script += "<div id='map' style='width:92%;height:400px;margin:auto;box-shadow: 0px 0px 10px 2px'></div>";					
